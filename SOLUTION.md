# Trusted Vendors Portal — Solution
...
- **No pagination:** The current dataset is small. If the vendor list grows to thousands of rows, server-side pagination with cursor-based queries would be the next step.
...
- **Dialog focus management:** The native `<dialog>` element handles some focus trapping automatically, but restoring focus to the trigger element on close required a custom `useDialog` composable that captures `document.activeElement` before opening.
